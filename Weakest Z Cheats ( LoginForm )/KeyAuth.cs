    ...
    public class api
    {
        ...
        public async Task enable2fa(string code = null)
        {
            ...
            if (json.success)
            {
                ...
                {
                    object value = System.Windows.Forms.MessageBox.Show("2FA has been successfully enabled!", "2FA Setup");
                }
            }
            else
            {
        ...
        private static void sigCheck(string resp, WebHeaderCollection headers, string type)
        {
            ...
            try
            {
                ...
                bool signatureValid = Ed25519.Verify(
                    byteSig, 0, byteKey, 0, byteBody, 0, byteBody.Length);
                if (!signatureValid)
                {
